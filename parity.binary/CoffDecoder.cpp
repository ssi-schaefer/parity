...
namespace parity
{
	...
	{
		//
		// to be consistent i use those too (since they are used in the original code)
		//
		#define DWORD int
		...
		int Decoder::decodeNext(void *position) {
			//
			// the following code is a little modified version of the one found
			// at http://hack-expo.void.ru/groups/blt/text/disasm.txt. Many thanks
			// to the original author (i don't know who he is, since the page is
			// all russian...
			//
			...
			DWORD disasm_defdata = 0;               // == C_66 ? 2 : 4
			DWORD disasm_defmem = 0;                // == C_67 ? 2 : 4
			...
			//
			// Here start the extensions to the original code.
			//
			Instruction insn;

			if(disasm_flag & C_66) insn.prefix66_ = true;
			if(disasm_flag & C_67) insn.prefix67_ = true;
			if(disasm_flag & C_LOCK) insn.prefixLock_ = true;
			if(disasm_flag & C_REP) insn.prefixRep_ = true;
			if(disasm_flag & C_SEG) insn.prefixSeg_ = true;
			if(disasm_flag & C_OPCODE2) insn.prefixOpcode2_ = true;
			if(disasm_flag & C_MODRM) insn.prefixModRm_ = true;
			if(disasm_flag & C_SIB) insn.prefixSib_ = true;

			insn.length_	= disasm_len;
			insn.segment_	= disasm_seg;
			insn.rep_		= disasm_rep;
			insn.opcode_	= disasm_opcode;
			insn.opcode2_	= disasm_opcode2;
			insn.modrm_		= disasm_modrm;
			insn.sib_		= disasm_sib;

			insn.memsize_	= disasm_memsize;
			insn.datasize_	= disasm_datasize;

			memcpy(insn.mem_, disasm_mem, sizeof(insn.mem_));
			memcpy(insn.data_, disasm_data, sizeof(insn.data_));
