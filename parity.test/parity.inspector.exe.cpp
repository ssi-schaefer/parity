...
namespace parity
{
	namespace testing
	{
		...
		bool TestSuite::testParityInspectorExe()
		{
			try {
				utils::Task::ArgumentVector exearguments;
				utils::Task::ArgumentVector libarguments;
				
				utils::Path maincfile = utils::Path::getTemporary(".parity.testsuite.main.inspector.XXXXXX.c");
				utils::Path libcfile = utils::Path::getTemporary(".parity.testsuite.library.inspector.XXXXXX.c");
				utils::Path oexefile = utils::Path::getTemporary(".parity.testsuite.inspector.XXXXXX.exe");
				utils::Path olibfile = utils::Path::getTemporary(".parity.testsuite.inspector.XXXXXX.so");
				...
				utils::Task inspector;
				utils::Task::ArgumentVector inspectorArgs;

				inspectorArgs.push_back("--verbose");
				inspectorArgs.push_back("-s");
				inspectorArgs.push_back(oexefile.get());

				std::ostringstream ossO;
				std::ostringstream ossE;

				inspector.setOutStream(ossO);
				inspector.setErrStream(ossE);

				if(!inspector.execute(getParityExecutable("parity.inspector"), inspectorArgs))
					throw utils::Exception("cannot execute parity inspector on output file.");

				dumpStreams(ossO.str(), ossE.str());
